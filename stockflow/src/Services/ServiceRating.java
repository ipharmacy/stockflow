...
public Rating getRating(int idrating)throws SQLException  
{
    ...
    @Override
     public void ajouter(Rating r) throws SQLException {      
        stee = con.createStatement();
        String requeteInsert = "INSERT INTO `gestion_entrepot`.`rating` (`idrating`,`idlivreur`, `note`,`iduser` ,`commentaire`  ) VALUES (NULL,  '" + r.getIdlivreur()+ "', '" + r.getNote()+"', '"+r.getIduser()+"', '"+r.getCommentaire()+ "');";
    ...
    ResultSet rs;
     ...
     while (rs.next()) {  
         Rating p=new Rating() ;
               ...
               p.setCommentaire(rs.getString(5))  ;
     arr.add(p);
     }
    ...
    public void deleteRatingId(int id) throws SQLException
    {
     ...
     PreparedStatement pre=con.prepareStatement( "UPDATE rating SET idlivreur=?, note=?,iduser=?,commentaire=? WHERE idrating=?");
        ...
        pre.setString(4, l.getCommentaire()) ; 
          pre.setInt(5, l.getIdrating()) ;
        
       
        pre.executeUpdate();
     }
     public List<Rating> recherRatingselonIdlivreur(int n) throws SQLException  
     {
         List<Rating> arr=new ArrayList<>();
    stee=con.createStatement();
    ResultSet rs=stee.executeQuery("select * from rating where idlivreur LIKE '"+n+"%'");
     while (rs.next()) {  
         Rating p=new Rating() ;
               p.setIdrating(rs.getInt(1));
               p.setIdlivreur(rs.getInt(2)) ; 
